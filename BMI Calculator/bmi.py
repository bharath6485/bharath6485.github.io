...
def BMI():
    ...
    #you can change value,beacuse different countries have different bmis
    if bmi<=18.5:
        label2.config(text="Underweight")
        label3.config(text="you have lower weight then normal body!")
    elif bmi>18.5 and bmi<=25:
        label2.config(text="Normal")
        label3.config(text="It indicates that your are healthy!")
    elif bmi>25 and bmi<=30:
        label2.config(text="overweight")
        label3.config(text="It indicates that a person is \n slightly overweight! \n A doctor may advice to lose some \n weight for health resons!")
    else:
        label2.config(text="obesi!")
...
#commaand to change background color of scale
style = ttk.Style()
